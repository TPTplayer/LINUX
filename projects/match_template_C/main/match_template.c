...
int match_template(char *path_image, char *path_template, char *path_result, int format, int *coordinate){
	int res = 0;
	FILE *image, *template;
	...
	res = match_template_ssd(image, template, path_result, format, coordinate);		
	if(res == -1){
		return -1;
	}
...
void normalization(float *data, int sx, int sy){
	float sum_n = 0.0F, std = 0.0F, mean = 0.0F;
	int n = sx * sy;
...
int match_template_ssd(FILE *image_file, FILE *template_file, char *name_result, int format, int *coordinate){
	FILE *result_file;
	...
	for(int y = 0; y < dst_sy; y++){
		for(int x = 0; x < dst_sx; x++){
			gdImageSetPixel(result, x, y, (int)(pxl_result[(y * image_sx) + x] * 0x2FFF));
		}
	}
	if(format == JPEG){
